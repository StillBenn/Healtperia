...
(function () {
  ...
  /* ---------- share modal ---------- */
  function openShare(){
    var url = location.href, title = (listing.headline || nm(TRm, listing.treatment_id) || 'Healthperia');
    var u = encodeURIComponent(url), t = encodeURIComponent(title + ' — Healthperia');
    var items = [['WhatsApp','wa','https://wa.me/?text=' + t + '%20' + u],['Facebook','fb','https://www.facebook.com/sharer/sharer.php?u=' + u],['X','x','https://twitter.com/intent/tweet?text=' + t + '&url=' + u],['Telegram','tg','https://t.me/share/url?url=' + u + '&text=' + t],['Instagram','ig',null]];
    var ov = document.createElement('div'); ov.className = 'td-share-overlay';
    ov.innerHTML = '<div class="td-share-modal" role="dialog" aria-modal="true"><button class="td-share-x" type="button" aria-label="' + esc(T('chat.close','Kapat')) + '">×</button>' +
      '<h4>' + esc(T('td.shareTitle','Bu ilanı paylaş')) + '</h4>' +
      '<div class="td-share-grid">' + items.map(function (it){ return '<button class="td-share-btn ' + it[1] + '" type="button" data-href="' + (it[2] || '') + '">' + shareIcon(it[1]) + '<span>' + esc(it[0]) + '</span></button>'; }).join('') + '</div>' +
      '<div class="td-share-copy"><input type="text" readonly value="' + esc(url) + '" /><button type="button" class="btn-primary sm td-copy">' + esc(T('td.copy','Kopyala')) + '</button></div></div>';
    document.body.appendChild(ov);
    function close(){ ov.remove(); }
    ov.addEventListener('click', function (e){ if (e.target === ov) close(); });
    ov.querySelector('.td-share-x').addEventListener('click', close);
    ov.querySelectorAll('.td-share-btn').forEach(function (b){ b.addEventListener('click', function (){ var href = b.dataset.href; if (href) window.open(href, '_blank', 'noopener,width=620,height=520'); else copy(url, b); }); });
